...
EMBEDDING_MODEL = os.getenv("FACE_MODEL", "buffalo_l")
...
@app.get("/health")
def health_check():
    return {"status": "ok", "service": "FaceFind Photos"}
...
def get_face_app():
    ...
    _face_app = FaceAnalysis(name=EMBEDDING_MODEL, providers=providers)
...
def extract_embeddings(image_path: Path) -> list[list[float]]:
    ...
    if image is None:
        raise HTTPException(400, f"Could not read image: {image_path.name}")
    ...
    try:
        ...
        raise HTTPException(
            503,
            "The face model is unavailable. On first launch it may need to download model files.",
        ) from exc
